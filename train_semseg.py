...
def main(args):
   ...
   # os.environ["CUDA_VISIBLE_DEVICES"] = args.gpu
    ...
    TRAIN_DATASET = MotorDataset(split='train', data_root=root, num_point=NUM_POINT, bolt_weight=args.bolt_weight, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    print("start loading test data ...")
    TEST_DATASET = MotorDataset(split='test', data_root=root, num_point=NUM_POINT, bolt_weight=args.bolt_weight, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            for i, (points, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
               ...
               # print("11111111111 labelweight after histogram", labelweights)
                ...
                for l in range(1, NUM_CLASSES):
                    ...
                    noBG_correct_class[l-1] += np.sum((pred_val == l) & (batch_label == l))
                    noBG_iou_deno_class[l-1] += np.sum(((pred_val == l) | (batch_label == l)))
